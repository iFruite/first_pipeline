...
public class MailManager {
    ...
    public SysSource feedMessage(Message message) throws IOException {
        ...
        try {
            ...
            if (isInvalidMessage(messageText, "Age")) {
                return null;
            }

            if (isInvalidMessage(messageText, "Skills")) {
                return null;
            }
        ...
        SysSource source = resumeFactory.parseEmail(mailId, messageText, resumeType, subject);
        if (source != null && subject != null) {
            Byte status = 0;
            source.setStatus(status);
            source.setReceiveDate(receiveDate);
            source.setPriority(flag ? "低" : "高");
            if (source.getPosition() != null) {
//                if (source.getSourceDetail().contains("测试") || source.getSourceDetail().contains("数据")) {
//                    source.setPriority("高");
//                }
                String position = source.getPosition();
                if (position != null ){
                    if (position.contains("财务") || position.contains("运营") || position.contains("部门助理")){
                        source.setPriority("中");
                    }
                }


            }
            if (source.getMobile() == null || source.getMobile().equals("")) {
                return null;
            }
     ...
     * @return
     */
    private boolean isInvalidMessage(String messageText, String keyFilter) {
        if (messageText != null) {
            ...
            Map ageMap = YamlHelper.getMapDict(mailConfig, keyFilter);
